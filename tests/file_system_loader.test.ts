...
import { CachingNodeFileSystemLoader } from "../src/loaders/file_system_loader";
...
describe("node file system loader", () => {
  ...
  test("async template not found", async () => {
    const loader = new NodeFileSystemLoader("tests/fixtures/templates");
    const env = new Environment({ loader });
    expect(async () => await env.getTemplate("nosuchthing.liquid")).toThrow(
      TemplateNotFoundError,
    );
  });
  ...
  test("async don't escape template search path", async () => {
    const loader = new NodeFileSystemLoader("tests/fixtures/templates");
    const env = new Environment({ loader });
    expect(async () => await env.getTemplate("../private.liquid")).toThrow(
      TemplateNotFoundError,
    );
    expect(
...
describe("caching node file system loader", () => {
  ...
  test("cached template", () => {
    ...
    expect(template).toBeInstanceOf(Template);
    expect(template.renderSync()).toBe("Hello, World!\n");
    expect(template.upToDateSync()).toBe(true);

    const sameTemplate = env.getTemplateSync("some.liquid");
    expect(sameTemplate).toBeInstanceOf(Template);
    expect(sameTemplate.upToDateSync()).toBe(true);

    expect(Object.is(template.nodes, sameTemplate.nodes)).toBe(true);
  });
  ...
  test("auto reload sync", () => {
    ...
    expect(template.upToDateSync()).toBe(false);
    ...
    expect(updatedTemplate.upToDateSync()).toBe(true);
    expect(Object.is(template.nodes, updatedTemplate.nodes)).toBe(false);
