...
You can pass configuration options as the second argument to [`Template.fromString()`](../api/classes/Template.md#fromstring). This example enables automatic HTML escaping and will throw an error at render time if a template attempts to use an undefined variable. See [`EnvironmentOptions`](../api/modules.md#environmentoptions) for all available options.
