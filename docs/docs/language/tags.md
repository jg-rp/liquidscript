...
is equivalent to `(true and (false and (false or true)))`, evaluating to `false`. JavaScript, on the
other hand, would parse an equivalent expression as `(((true & false) & false) | true)`, evaluating
to `true`.
...
Render text containing literal `{{`, `}}`, `{%` and `%}` strings. Everything inside a `raw` block will not be parsed as a Liquid template.
...
:::info
A [forloop](#forloop) object is available inside templates rendered with the `for` / `as` syntax. `forloop.parentloop` will always be undefined.
:::
