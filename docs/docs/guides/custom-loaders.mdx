...
The `MongoDBLoader` constructor expects a MongoDB `Collection` from an already connected client, and that documents in that collection have a `uid` and `templates` field. Fields in the embedded `templates` document are Liquid template names and their values are Liquid template source text.
