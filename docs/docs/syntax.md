---
sidebar_label: Liquid syntax
---
# Introduction to Liquid template syntax
...
`{{ site_description }}` and `{{ item.title | capitalize }}` are examples of [output statements](./reference/tags.md#output). Expressions surrounded by double curly braces, `{{` and `}}`, will be evaluated and the result inserted into the output text.

## Filters

`capitalize` in `{{ item.title | capitalize }}` and `ceil` in `{{ item.price | ceil }}` are examples of [filters](./reference/filters.md). Filters modify the expression to their left prior to output or assignment.

## Tags

`{% if site_description %}`, `{% endif %}` and `{% assign items_size = items | size %}` are examples of [tags](./reference/tags.md). After the start tag delimiter (`{%`) there must be a tag name. Everything up to the closing tag delimiter (`%}`) is the tags's expression.

Not all tags accept an expression, but all tag must have a name.

Together `{% if site_description %}` and `{% endif %}` form a _block tag_. Block tags have an opening tag, some Liquid in between, and an end tag. In the case of the [`if tag`](./reference/tags.md#if), the block is only rendered if the tag's expression evaluates to true.
...
`{% # About us section %}` is an example of a [comment](./reference/tags.md#comments). Text between `{% #` and `%}` will not be parsed or rendered.
