...
import { Environment, ObjectLoader } from "../dist";

class Fixture {
  constructor(
    readonly name: string,
    readonly data: { [index: string]: unknown },
    readonly templates: { [index: string]: string },
  ) {}

  static load(fixture: string): Fixture {
    const name = path.basename(fixture);
    const templates: { [index: string]: string } = {};
...
const benchOptions: { [index: string]: unknown } = {
